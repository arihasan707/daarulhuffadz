$(document).ready( function (){
	...
	//INSTITUTE PRODI
	$("select[name='sistem']").on('change', () => {
		const h = $("select[name='sistem'] option:selected").val();
		if (h === '1') {
			$("select[name='prodi']").removeAttr('disabled');
			$(".tri").remove();
			$("select[name='prodi']").append($('<option>').val('1').text('PAI - Pendidikan Agama Islam').addClass('one'));
			$("select[name='prodi']").append($('<option>').val('2').text('PAI & IAT (Double Gelar)').addClass('two'));
		} else if(h === '2'){
			$("select[name='prodi']").removeAttr('disabled');
			$('.one').remove();
			$('.two').remove();
			$("select[name='prodi']").append($('<option>').val('3').text('PAI & IAT (Double Gelar)').addClass('tri'));
		}
		else {
			$(".one").remove();
			$(".two").remove();
	...
	$("#karantina").validate({
		...
		messages: {
			...
			tes: {
				...
				extension: "<div class='validasi'>File harus bertipe mp3/aac/wav !</div>",
			...
			bayar: {
				...
				extension: "<div class='validasi'>File harus bertipe png/jpg/jpeg !</div>",
				fileSize: "<div class='validasi'>File terlalu besar. Max 2 Mb !</div>",
			},
	...
	$("form#institute").validate({
		...
		messages: {
			...
			prodi: "<div class='validasi'>Program studi harus di isi !<div>",
			nama: "<div class='validasi'>Nama lengkap harus di isi !<div>",
			...
			hasil_ayah: "<div class='validasi'>Penghasilan perbulan harus di isi !<div>",
			hp_ayah: "<div class='validasi'>No WA ayah harus di isi !<div>",
			// ibu
			...
			hasil_ibu: "<div class='validasi'>Penghasilan perbulan harus di isi !<div>",
			hp_ibu: "<div class='validasi'>No WA ibu harus di isi !<div>",
	...
	$("form#boarding").validate({
		rules: {
			...
			ktp_ayah: { required: true, extension: "png|jpg|jpeg", fileSize: 2000000 },
			ijazah: { required: true, extension: "png|jpg|jpeg", fileSize: 2000000 },
	...
	$("form#dauroh").validate({
		...
		$('#prog').on('change', function () {
			...
			if(selectVal === "1"){
				...
				$("#jur").parent().parent().children().children().removeClass("col-md-12");
